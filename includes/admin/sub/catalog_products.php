...
if(!defined('WORKING_THROUGH_ADMIN_SCRIPT'))
{
	...
	if (!strcmp($sub, "products"))
	{
		...
		if (isset($_GET["dublicate_product"]) && $_GET["dublicate_product"]=(int)$_GET["dublicate_product"]) { 
                        ...
                        if ($access_dubl)
                        { 
                        $par=(int)$_GET["categoryID"]; $_GET["dublicate_product"]=(int)$_GET["dublicate_product"];
			...
			$pid = db_insert_id();	if ($ar['enabled']) $cout_p=1; else $cout_p=0;
		...
		$par=$categoryID; $patch_category=array();
                while($par > 0)
                {
