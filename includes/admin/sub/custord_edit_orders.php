  ...
  if(!defined('WORKING_THROUGH_ADMIN_SCRIPT'))
{
	...
	if (!strcmp($sub, "edit_orders"))
	{
            ...
            {  
              ...
              $needle=ADMIN_SHIPPING.' '; $haystack=$q['name']; $Quantity=0;
              ...
              header("Location: admin.php?dpt=custord&sub=edit_orders&orderID=".(int)$_GET['orderID']);
            } 
            if (isset($_POST['save_order']))
            {
              ...
              if (isset($_POST['dicount'])) 
              {
               ...
               if ($key==0 && $_POST['dicount'][$key]['Price']>0) 
                 add_field(ORDERED_CARTS_TABLE, $_POST['dicount'][$key]);
               elseif ($key!=0)
               {
              ...
              foreach ($_POST["addproduct"] as $key => $value)
              {
                 ...
                 $value['name']=$p_inf['name']; $p['in_stock']=$p_inf['in_stock']-$value['Quantity'];
