...
if (!strcmp($sub, "new_orders")) {
    ...
    unset($sql_new_order);
    $q = db_query('SELECT OC.orderID, OC.Price FROM ' . ORDERS_TABLE . ' AS O INNER JOIN ' . ORDERED_CARTS_TABLE . ' AS OC USING ( orderID ) WHERE OC.name LIKE \'' . ADMIN_DISCOUNT_STRING . '%\' and O.status =1 GROUP BY O.orderID');
    while ($row = db_fetch_row($q)) $diskont[$row[0]] = $row[1];
