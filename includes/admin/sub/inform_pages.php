...
if(!defined('WORKING_THROUGH_ADMIN_SCRIPT'))
{
	...
	if (!strcmp($sub, "pages"))
	{
		...
		if (isset($_GET["del_page"])) // delete page
		{
			...
			header("Location: admin.php?dpt=inform&sub=pages");
		}
		...
		if (isset($_POST["add_page"]))
		{
			...
			header("Location: admin.php?dpt=inform&sub=pages&pages=".$_POST["add_page"]);
		}
		...
		if (isset($_GET["picture_remove"]) && isset($_GET["pages"]) && ($_GET["pages"])) // remove picture
		{
			...
			header("Location: admin.php?dpt=inform&sub=pages&pages=".$_GET["pages"]);
		}
