...
****************************************************************************/
    if (!count($_GET) && !count($_POST))
    {
        ...
        $total=db_assoc('SELECT sum( OC.Price * OC.Quantity ) revenue, count(O.orderID) orders FROM `'.ORDERS_TABLE.'` AS O INNER JOIN `'.ORDERED_CARTS_TABLE.'` AS OC USING ( orderID )');
