...
****************************************************************************/
	...
	if (isset($_POST["complete_order"]) || (isset($_GET["order_placement_result"]) && isset($_SESSION['order_reg']))) //place order
	{
		...
		if (isset($_SESSION["gids"]) && $c)
		{
			...
			for ($i=0; $i<count($_SESSION["gids"]); $i++)
			  if ($_SESSION["gids"][$i])
			  {
				...
				$q = db_query("SELECT P.name, Price, product_code, hurl, categoryID,email manager_email, online_name FROM ".PRODUCTS_TABLE.' as P LEFT JOIN '.MANAGER_TABLE." as M on (M.ID=P.managerID) WHERE productID='".$_SESSION["gids"][$i]."'") or die (db_error());
				if ($r = db_fetch_row($q))
				{
                                ...
                                { 
                                  ...
                                  for( $ti=0; $ti<count($tmp['option']); $ti++)
                                  {
                                   ...
                                   $v=db_assoc("select povalt.name name, povt.count count from ".PRODUCT_OPTIONS_VAL_TABLE.' as povalt left join '.PRODUCT_OPTIONS_V_TABLE.' as povt on povt.variantID=povalt.variantID  where povalt.variantID='.$tmp['variant'][$ti].' and productID='.$_SESSION["gids"][$i]);   
                                   $variants .=':'.$v['name']; 
                                   $udate_varian_count=array();
                                   $udate_varian_count['count']=$v['count']-$_SESSION["counts"][$i];
                                   update_field(PRODUCT_OPTIONS_V_TABLE, $udate_varian_count,'variantID='.$tmp['variant'][$ti]);
		...
		//	  }
			...
			$smarty_mail->assign("order_shipping_address", "ã.".$_POST["city"]."\n".$_POST["address"]); //."\nã.".." ".$_POST["state"]."  ".$_POST["zip"]."\n".$_POST["country"]
