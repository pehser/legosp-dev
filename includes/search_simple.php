   ...
   return $categ_id;
	...
	// simple search
        if (isset($_GET["search_expanded_ajax"]))
        {
           ...
           if (DB_CHARSET != 'utf8') $html=win2utf($html);
           ...
           if  (isset($_POST['search']))  
           {
             ...
             $sql_count .='select count(*) FROM `' . PRODUCTS_TABLE . '` AS P INNER JOIN `' . CATEGORIES_TABLE . '` AS C USING(categoryID) WHERE P.enabled =1 AND C.enabled =1';
              ...
              {    
                   ...
                   $sql_count .= ' and categoryID in ('.$categ_in.')';
              } 
              
	      
              if(get_magic_quotes_gpc()) $_POST['keyword']=stripslashes($_POST['keyword']);
              $keyword=mysql_real_escape_string($_POST['keyword']);  
              if  ($keyword != STRING_SEARCH) {$sql .=' and P.name like \'%'.$keyword.'%\'';  $sql_count .=' and P.name like \'%'.$keyword.'%\'';}
	...
	elseif (isset($_GET["searchstring"])) //make a simple search
	{
		...
		if ($_GET["searchstring"])
		{
			...
			$smarty->assign("sort_options", $sort_options);
                        ...
                        $g_search_count=db_r('select count(*) FROM `' . PRODUCTS_TABLE . '` AS P INNER JOIN `' . CATEGORIES_TABLE . '` AS C USING(categoryID) WHERE P.enabled =1 AND C.enabled =1 and (P.name LIKE \'%'.$search[0].'%\' or P.description  LIKE \'%'.$search[0].'%\' or P.product_code LIKE \'%'.$search[0].'%\' or P.brief_description LIKE \'%'.$search[0].'%\')');  
                        $sql = 'SELECT P.*,B.name brand_name, B.hurl brand_hurl, count(R.reviewID) count_review FROM `' . PRODUCTS_TABLE . '` AS P LEFT JOIN `' . REVIEW_TABLE . '` as R USING(productID) LEFT JOIN `' . BRAND_TABLE . '` as B USING (brandID)  INNER JOIN `' . CATEGORIES_TABLE . '` AS C ON (P.categoryID = C.categoryID) WHERE P.enabled =1 AND C.enabled =1 and (P.name LIKE \'%'.$search[0].'%\' or P.description  LIKE \'%'.$search[0].'%\' or P.product_code LIKE \'%'.$search[0].'%\' or P.brief_description LIKE \'%'.$search[0].'%\') GROUP BY `P`.`productID` ORDER BY P.' . $_SESSION['sort'] . ' ' . $_SESSION['order']; 
